struct trie{
	...
	int idhoja, sihoja;//id de la hoja o 0 si no lo es
	...
	trie *get_nxthoja(){
		...
		return nxthoja;	}
	void print(int p){
		if(idhoja) cout << "found " << idhoja << "  at position " << p-sihoja << endl;
		if(get_nxthoja()) get_nxthoja()->print(p); }
	void matching(const string &s, int p=0){
		print(p); if(p<si(s)) get_tran(s[p])->matching(s, p+1);	}
}tri;
